...
Now, we need to add our data files and metadata to the DataPackage. NMFS requires
that all metadata be cataloged within the InPort application. The
`kotzeb0912_raw_iso19115.xml` file is an ISO 19115-2 export from InPort. The 
DataPackage also includes a `wc_archive_files_iso19115.xml` file that
describes the individual files that are found within a Wildlife Computers
deployment archive.

> **[JML comment]** the examples and vignettes online all describe using EML and functions w/in
the eml package. Not sure what the best workflow is when the metadata are not
in EML format.
...
```{r connect-D1}
# we'll use a knb test server to start with
